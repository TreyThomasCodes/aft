...
4. Provision governed agent child environment (`crates/aft/src/agent_child_env.rs`) for spawned bash/PTY processes, populating a shims directory containing `gh` routing shim and a generated POSIX `prepare-commit-msg` git co-author hook (`AFT_GIT_CO_AUTHOR`), attaching attribution without altering user shell files or global git config.
...
- Pattern: Shims directory manager and environment builder provisioning `gh` routing shim and a generated POSIX `prepare-commit-msg` hook for joint co-authorship attribution (`AFT_GIT_CO_AUTHOR`) without modifying repository or global Git configuration.
